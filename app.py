...
def predict():
    ...
    row = []
    for x in range(1,31):
        y = request.form.get(f'attribute {x}')
        row.append(float(y))
    ...
    return render_template("index.html",
                prediction_text_relu = "{}".format(prediction_relu),
                prediction_text_sigmoid = "{}".format(prediction_sigmoid),
                prediction_text_logistic = "{}".format(prediction_logistic))
